...
if __name__ == '__main__':
    ...
    X_train, Y_train, p_test, q_test = prepared_data()
    ...
    if check_file(origin_model_path):
        ...
        model = train_model(model, data_1_train, data_1_label, origin_model_path)
    evaluate_model(model, X_test, Y_test)

    # 第二次模型训练
    model = update_model(origin_model_path)
    model = train_model(model, X_train, Y_train, new_model_path)
    evaluate_model(model, X_test, Y_test)
